...
router: APIRouter = APIRouter(
    prefix="/oauth2-token/v1", tags=["account-manager.oauth2-token.v1"]
)
...
def get_tokens(
    ...
    tokens: list[OAuth2AccessTokenDocument] = OAuth2AccessTokenDocument.objects(
        subject=account.id
    )
    return [
        Token.build(token, OAuth2ClientDocument.objects(id=token.audience).first())
        for token in tokens
    ]
...
def delete_token(
    token_id: str, session: SessionDocument = Depends(SessionService.get_required)
):
    account: AccountDocument = AccountService.get_by_session(session)
    token: Optional[OAuth2AccessTokenDocument] = OAuth2AccessTokenDocument.objects(
        id=token_id, subject=account.id
    ).first()
    if not token:
        raise APIException(400, "invalid_token", "Invalid token id")
    refresh_token: OAuth2RefreshTokenDocument = OAuth2RefreshTokenDocument.objects(

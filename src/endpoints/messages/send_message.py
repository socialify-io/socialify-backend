...
from app import socketio, user_session, app
...
def connect():
    try:
        ...
        emit('connection_status', ErrorResponse(
    ...
    if verify_authtoken(headers, "connect"):
        ...
        if verify_sign(request, pub_key, "connect"):
            ...
            data_to_update = dict(messageToken=message_token)
            user_session.query(Device).filter(Device.userId == userId).filter(Device.fingerprint == headers['Fingerprint']).update(data_to_update)
            user_session.commit()

            emit(Response(data={'messageToken': message_token}).__dict__)
        else: 
            error = ApiError(
